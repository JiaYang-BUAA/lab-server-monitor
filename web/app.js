...
(() => {
  ...
  function renderServers() {
    ...
    const disclosureStates = new Map([...$('server-list').querySelectorAll('details[data-preserve]')].map((node) => [`${node.closest('[data-host]')?.dataset.host}:${node.dataset.preserve}`, node.open]));
    const previousChart = $('server-list').querySelector('.chart-data-scroll');
    const previousChartHost = previousChart?.closest('[data-host]')?.dataset.host;
    const previousChartScroll = previousChart?.scrollTop;
    ...
    $('server-list').querySelectorAll('details[data-preserve]').forEach((node) => {
      const key = `${node.closest('[data-host]')?.dataset.host}:${node.dataset.preserve}`;
      node.open = disclosureStates.has(key) ? disclosureStates.get(key) : node.hasAttribute('data-default-open');
    });
    const currentChart = $('server-list').querySelector('.chart-data-scroll');
    if (currentChart && previousChartHost === selectedId && previousChartScroll != null) currentChart.scrollTop = previousChartScroll;
  }
